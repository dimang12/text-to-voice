...
type Props = { counts: { history: number; voices: number; bookmarks: number; scripts: number } };
...
export function WorkspaceTabs({ counts }: Props) {
  ...
  return (
    <nav className="tabs" role="tablist">
      {tabs.map((tab) => (
        <Link key={tab.href} href={tab.href} className={`tab ${path === tab.href ? "active" : ""}`} role="tab" aria-selected={path === tab.href}>
          {tab.label}
